 ...
 *                                                                Terry Louwers
 *
 * \brief   Unit tests for the ExampleProject AppLogic class.
 *
 * \note    https://github.com/tlouwers/STM32F4-DISCOVERY/tree/develop/ExampleProject/tests
 *
 * \details Exercises the interface-injected application logic with Mock
 *          drivers (Mock_LIS3DSH, Mock_Watchdog, Mock_Pin). Application
 *          itself is the composition root and is intentionally not tested.
 *
...
protected:
    ...
    Mock_Watchdog mWatchdog;
    Mock_Pin      mMotionLed;

    AppLogic mSubject { mAccelerometer, mWatchdog, mMotionLed };
};
...
TEST_F(AppLogic_Test, ServiceTick_NormalWakePercentage_RefreshesWatchdog)
{
    EXPECT_CALL(mWatchdog, Refresh()).Times(1);

    mSubject.ServiceTick(10.0f);
}

TEST_F(AppLogic_Test, ServiceTick_HighWakePercentage_StillRefreshesWatchdog)
{
    EXPECT_CALL(mWatchdog, Refresh()).Times(1);

    mSubject.ServiceTick(99.0f);
}
