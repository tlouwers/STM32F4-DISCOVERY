 ...
 *          guard. It depends only on driver interfaces (ILIS3DSH, IWatchdog,
 *          IPin) so it can be unit-tested with Mock drivers, while Application
...
{
public:
    AppLogic(ILIS3DSH& accelerometer, IWatchdog& watchdog, IPin& motionLed);

    void OnMotionData(uint8_t length);
    void ServiceTick(float wakePercentage);
    void Process();
