...
TEST_F(LIS3DSH_Test, Init_IsInit_Sleep)
{
    ...
    EXPECT_TRUE(mSubject.Init(LIS3DSH::Config(false,
...
{
    EXPECT_TRUE(mSubject.Init(LIS3DSH::Config(true,
                                              LIS3DSH::SampleFrequency::_50_Hz,
                                              LIS3DSH::Scale::_2_G,
                                              LIS3DSH::AntiAliasingFilter::_200_Hz)));

    uint8_t motionArray[25 * 3 * 2] = {};       // 25 samples, X,Y,Z, 2 bytes/sample -- FIFO size
    uint8_t motionLength = sizeof(motionArray); // Normally returned from interrupt when data received
