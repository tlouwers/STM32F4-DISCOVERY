...
* FreeRTOS as library, shared kernel source at repository root, per-project configuration in target/Src/.
...
* target - This folder contains the Cortex-M4 (ARM) project to be build (Cross-Compiled). The 'build' folder contains the artifacts created. The 'Src' folder contains the user code, drivers, application logic, and FreeRTOSConfig.h. The 'Startup' folder contains the startup assembly file. The 'arm-none-eabi-gcc.cmake' file contains setting to Cross-Compile for ARM. The file 'gcc-options-cxx.txt' contains specific GCC flags. They are present here as not to clutter the CMake file. CMSIS and STM32 HAL code are in the shared 'hal/' folder at the repository root.
* target/FreeRTOS - CMakeLists.txt that builds the FreeRTOS library from the shared 'freertos/' folder at the repository root.
...
* In folder 'target/Src' is the FreeRTOSConfig.h configuration file for the Cortex-M4 board.
